...
class NewsArticle:
    ...
    status: str = "new"  # new, processed, published
...
class NewsMonitor:
    ...
    def _initialize_sources(self) -> List[Dict]:
        """Initialize news sources for monitoring"""
        return [
            # Music Publications
            {
                "name": "Pitchfork",
            ...
            {
                "name": "Stereogum",
                "search_url": "https://www.stereogum.com/?s={query}",
                "type": "publication",
                "selector": ".post"
            },
            {
            ...
            {
                "name": "Reddit Music",
                "search_url": "https://www.reddit.com/r/Music/search/?q={query}",
                "type": "social",
                "selector": ".Post"
            },
            {
    ...
    def search_for_mentions(self, artist: str, limit: int = 5) -> List[NewsArticle]:
        ...
        for source in self.search_sources[:3]:  # Limit sources for demo
            try:
                ...
                if response.status_code == 200:
                    ...
                    for article_elem in articles:
                        try:
                            ...
                            # Only include if it mentions our artist
                            if artist.lower() in title.lower():
                                article = NewsArticle(
                                    id="",  # Will be generated
                                    title=title,
                                    content=article_elem.get_text(strip=True)[:500],
                                    source=source['name'],
                                    url=search_url,  # Simplified - would extract actual article URL
                                    artist_mentioned=[artist],
                                    article_type="review" if "review" in title.lower() else "news"
                                )
    ...
    def collect_news(self, max_per_artist: int = 3):
        ...
        # Add mock articles for demonstration
        mock_articles = self._generate_mock_articles()
        
        self.articles.extend(new_articles + mock_articles)
        self.save_articles()
        ...
        logging.info(f"🎉 Collected {len(new_articles)} new articles ({len(mock_articles)} demo articles)")
        return len(new_articles) + len(mock_articles)
    ...
    def _generate_mock_articles(self) -> List[NewsArticle]:
        ...
        mock_data = [
            {
                "title": "My Evil Robot Army's Space Jazz EP Blends Lo-Fi with Experimental Electronic",
                "content": "The latest release from My Evil Robot Army showcases a unique fusion of jazz elements with lo-fi production techniques. The Space Jazz EP demonstrates the project's evolution toward more sophisticated electronic compositions while maintaining its signature experimental edge.",
                "source": "Electronic Music Weekly",
                "artist_mentioned": ["My Evil Robot Army"],
                "sentiment": "positive",
                "article_type": "review"
            },
            {
                "title": "MERA's 'Explorations in Blue' Receives Critical Acclaim",
                "content": "MERA's ambient electronic album 'Explorations in Blue' has been praised by critics for its immersive soundscapes and innovative use of synthesized textures. The album represents a significant step forward in the artist's sonic exploration.",
                "source": "Ambient Music Review",
                "artist_mentioned": ["MERA"],
                "sentiment": "positive", 
                "article_type": "review"
            },
            {
                "title": "NullRecords: The Independent Label Pushing Electronic Music Boundaries",
                "content": "NullRecords has emerged as a compelling force in the independent electronic music scene, with a roster that challenges conventional genre boundaries. Their artists consistently deliver innovative compositions that blend traditional and digital elements.",
                "source": "Independent Music Today",
                "artist_mentioned": ["NullRecords"],
                "sentiment": "positive",
        ...
        for data in mock_data:
            article = NewsArticle(
                ...
                source=data["source"],
                url=f"https://example.com/articles/{data['title'].lower().replace(' ', '-')}",
                artist_mentioned=data["artist_mentioned"],
